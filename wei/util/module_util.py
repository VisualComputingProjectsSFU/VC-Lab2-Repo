...
def forward_from(module_seq: nn.Sequential,
    ...
    :param start_idx: start index of forwarding layer.
...
def summary_layers(model, input_size):
    def register_hook(reg_module):
        def hook(hook_module, inp, outp):
            class_name = str(hook_module.__class__).split('.')[-1].split("'")[0]
            module_idx = len(summary)
            if hasattr(hook_module, 'tag'):
                module_tag = getattr(hook_module, 'tag')
            else:
                module_tag = ''
            ...
            summary[m_key]['input_shape'] = list(inp[0].size())
            summary[m_key]['input_shape'][0] = -1
            if isinstance(outp, (list, tuple)):
                summary[m_key]['output_shape'] = [[-1] + list(o.size())[1:] for o in outp]
            else:
                summary[m_key]['output_shape'] = list(outp.size())
                summary[m_key]['output_shape'][0] = -1

            params = 0
            if hasattr(hook_module, 'weight'):
                params += torch.prod(torch.LongTensor(list(hook_module.weight.size())))
                summary[m_key]['trainable'] = hook_module.weight.requires_grad
            if hasattr(hook_module, 'bias') and hasattr(hook_module.bias, 'size'):
                params += torch.prod(torch.LongTensor(list(hook_module.bias.size())))
            summary[m_key]['nb_params'] = params

        if (not isinstance(reg_module, nn.Sequential) and
                not isinstance(reg_module, nn.ModuleList) and
                not (reg_module == model)):
